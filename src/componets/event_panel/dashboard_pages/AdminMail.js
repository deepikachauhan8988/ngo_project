...
const AdminMail = () => {
  ...
  const fetchMembers = async () => {
    setIsLoading(true);
    try {
      // Make authenticated request using authFetch to get all members (no district filter)
      const response = await authFetch('https://mahadevaaya.com/ngoproject/ngoproject_backend/api/member-reg/', {
        method: 'GET',
        headers: {
  ...
  const fetchAdminMailData = async () => {
    setIsLoading(true);
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || errorData.detail || 'Failed to fetch admin mail data');
      }
      
      const data = await response.json();
      console.log('Admin mail data:', data);
      
      // Set admin ID and name
      if (data.admin_id) {
        setAdminId(data.admin_id);
        setFormData(prev => ({ ...prev, admin_id: data.admin_id }));
      }
      ...
      console.error('Error fetching admin mail data:', error);
      setErrors({ fetch: error.message || "Failed to fetch admin mail data. Please try again." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        admin_id: formData.admin_id || auth?.unique_id || adminId,
