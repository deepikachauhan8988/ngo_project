...
const GetAdminMail = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      // Fetch admin mail data
      const adminMailResponse = await authFetch('https://mahadevaaya.com/ngoproject/ngoproject_backend/api/admin-mail/', {
        method: 'GET',
        headers: {
      ...
      if (!adminMailResponse.ok) {
        if (adminMailResponse.status === 401) {
          throw new Error('Session expired. Please login again.');
        }
        if (adminMailResponse.status === 403) {
          throw new Error('Permission denied. You may not have the required role to access this feature.');
        }
        throw new Error(`Failed to fetch admin mail data. Status: ${adminMailResponse.status}`);
      }
      
      const adminMailResult = await adminMailResponse.json();
      console.log("Admin Mail API Response:", adminMailResult);
      ...
      let mails = [];
      if (Array.isArray(adminMailResult)) {
        mails = adminMailResult;
      } else if (adminMailResult && adminMailResult.success && adminMailResult.data) {
        mails = Array.isArray(adminMailResult.data) ? adminMailResult.data : [adminMailResult.data];
      } else if (adminMailResult && typeof adminMailResult === 'object') {
        mails = [adminMailResult];
      }

      const latestMail = mails.length > 0 ? mails[mails.length - 1] : {};
      console.log('Selected admin mail (latest):', latestMail);

      // Keep latest mail for the details card, but keep full mails array for history
      setAdminMailData({
        admin_id: latestMail.admin_id || latestMail.admin || "",
                      ...
                      <Table responsive striped hover>
                        ...
                        <tbody>
                          {matchedMembers.map((record, index) => {
                            ...
                            const mail = record && record.mail ? record.mail : adminMailData;
                            return (
